...
## デバッグ右クリックループ
共有済みの次の座標を使って、順番に右クリックします。

- `normal_chest`: `x=812`, `y=942`
- `boss_chest`: `x=781`, `y=944`

デフォルトでは 3 秒待機してから開始し、10 秒ごとに 2 箇所を順番に右クリックします。
...
```bash
py -3 scripts\run_debug_right_click_loop.py --startup-delay-seconds 5 --interval-seconds 10
```

実行中は各クリックの対象名、座標、時刻がターミナルに表示されます。停止したいときは `Ctrl+C` です。
