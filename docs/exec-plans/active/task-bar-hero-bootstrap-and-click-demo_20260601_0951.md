...
## 実装ステップ
- [ ] Git 管理を初期化し、初回 push の前提を整える
- [ ] Windows で使う Python 依存と `.gitignore` を追加する
- [ ] クリック座標取得デモを `scripts/` と `src/` に実装する
- [ ] 実行手順と返却フォーマットを README に記載する
- [ ] 最低限の動作確認を行う
- [ ] 計画ファイルを `completed/` に移動し、実装をコミット・push する
